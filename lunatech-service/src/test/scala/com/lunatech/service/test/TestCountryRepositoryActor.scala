...
import com.lunatech.service.repository.CountryRepositoryActor
...
class TestDocumentRepositoryActor(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with BeforeAndAfter with BeforeAndAfterAll with ScalaFutures with ImplicitSender {
  ...
  def target() = _system.actorOf(Props(classOf[CountryRepositoryActor]), "CountryRepositoryActor")
